...
Linux에서 사용자와 그룹의 정보가 `/etc/passwd`와 `/etc/group`에 어떻게
저장되는지 배우고, 사용자를 생성·삭제하거나 그룹에 소속시키는 방법을
실습하였다. 또한 `chown`과 `chmod`를 이용하여 파일의 소유권과
읽기·쓰기·실행 권한을 변경하고, 여러 사용자로 직접 전환하면서 권한에
따라 접근 결과가 달라지는 것을 확인하였다.

프로세스에서는 프로그램과 프로세스의 차이, 부모·자식 관계, 프로세스
상태와 스케줄링, 컨텍스트 스위칭, 스레드의 개념을 학습하였다. `ps`,
`kill`, `killall`을 이용한 프로세스 관리와 `&`, `Ctrl+Z`, `bg`, `jobs`를
이용한 작업 제어도 실습하였다. 또한 표준 스트림과 파이프, 메시지 큐,
소켓 등의 프로세스 간 통신 방법도 학습하였다.

#### 느낀점

처음에는 `chmod`의 숫자와 문자 표기법, 소유자·그룹·기타 사용자의 권한을
구분하는 것이 헷갈렸지만 여러 사용자로 직접 전환해 파일을 읽고 수정해
보면서 권한이 실제로 어떻게 적용되는지 이해하기 쉬웠다. 특히
`Permission denied`와 `Operation not permitted`가 발생하는 상황을 직접
확인하면서 파일과 디렉터리의 권한을 정확하게 설정하는 것이 중요하다는
것을 알게 되었다.

프로세스 부분은 명령어뿐만 아니라 부모·자식 관계, 상태, 스케줄링처럼
새로운 개념이 많아 조금 복잡하게 느껴졌다. 하지만 `sleep`과 `ping`을
직접 실행하고 프로세스를 조회하거나 종료하고, 포어그라운드 작업을
백그라운드로 전환해 보면서 프로세스가 실제로 어떻게 관리되는지 조금 더
이해할 수 있었다.
