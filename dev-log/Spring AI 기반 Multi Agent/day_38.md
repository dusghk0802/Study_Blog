...
| :---: | :---: |
| **isEqualTo(3)일 경우** | **isEqualTo(2)일 경우** |
...
처음에는 테스트 코드가 단순히 오류가 있는지 확인하는 용도라고 생각했지만, 예상값을 일부러 다르게 설정해 테스트가 실패하는 과정까지 직접 확인하면서 테스트의 목적을 더 쉽게 이해할 수 있었다. 
</br>특히 성공, 반복 실행, 비활성화, 실패 결과가 IntelliJ에서 각각 다르게 표시되는 것을 확인해 보니 테스트 코드가 프로그램의 기능이 의도한 대로 동작하는지 검증하는 데 중요하다는 것을 알게 되었다.
