...
심볼릭 링크와 하드 링크를 직접 생성하고 stat, ls -li 명령어를 사용해 inode와 링크 정보를 확인하였다. 
</br>특히 심볼릭 링크는 원본 파일의 위치가 변경되면 연결이 끊어지지만, 하드 링크는 원본과 같은 inode를 공유한다는 차이를 확인하였고 명령어로 직접 파일을 이동하고 링크가 어떻게 변하는지 확인해 보니 심볼릭 링크와 하드 링크의 차이를 개념으로만 배울 때보다 쉽게 이해할 수 있었다.
