...
systemd 서비스 실행 오류가 발생했을 때 systemctl status와 journalctl을 통해 오류 원인을 확인하고, 서비스 파일과 실행 스크립트의 경로 및 실행 권한을 점검하는 방법을 배웠다.

단순히 서비스를 다시 실행하는 것보다 오류 메시지를 확인하고 원인을 하나씩 수정하는 과정이 중요하다는 것을 알게 되었다.
