...
터미널과 셸의 차이를 배우고 Linux에서 명령어를 이용해 파일과 디렉터리를
다루는 기본적인 방법을 실습하였다. `ls`, `cd`, `pwd`, `cat`, `mkdir`
등의 명령어를 직접 사용하면서 디렉터리를 이동하고 현재 위치를 확인하는
방법을 익혔다. 또한 상대 경로와 절대 경로의 차이와 Linux의 파일 시스템이
계층 구조로 구성되어 있다는 것을 배웠다.

#### 느낀점

처음에는 명령어만 사용해서 파일과 디렉터리를 다루는 것이 익숙하지 않았고
현재 어느 위치에서 작업하고 있는지 헷갈리기도 했다. `pwd`로 현재 위치를
확인하고 `ls`와 `cd`를 반복해서 사용해 보면서 조금씩 디렉터리 구조를
이해할 수 있었다. 특히 `mkdir -p`처럼 옵션에 따라 명령어의 동작이
달라지는 것을 직접 확인해서 기본 명령어와 옵션을 함께 익혀야겠다고
느꼈다.
