...
디렉터리를 이동하거나 이름을 입력할 때 철자를 잘못 작성하면 의도하지 않은 디렉터리가 생성되거나 잘못된 경로에서 작업할 수 있다는 것을 확인하였다. 
<br/>ls를 통해 잘못 생성된 animlas 디렉터리를 확인하고, 일반 rm으로 삭제되지 않는 것을 확인한 뒤 rm -r animlas 명령어를 사용하여 잘못된 디렉터리를 삭제하는 방법을 배웠다.
<br/>단순한 철자 실수 하나로도 의도하지 않은 디렉터리가 만들어질 수 있다는 점이 기억에 남았다. 
<br/>오류가 발생했을 때 바로 명령어를 다시 입력하기보다는 pwd나 ls로 현재 위치와 파일·디렉터리 상태를 먼저 확인하는 습관이 중요하다고 느꼈다.
