...
JSX를 이용해 화면을 구성하고 반복되는 부분을 함수형 컴포넌트로 분리하는
방법을 배웠다. Props로 부모에서 자식에게 문자열, 배열, 함수 등을
전달하고 구조분해할당으로 필요한 값만 사용하는 방법을 실습하였다. 또한
이벤트를 이용한 데이터 전달과 `preventDefault()`, 컴포넌트의
`export default`와 `import`, `children`을 이용한 재사용 방법도 배웠다.

#### 느낀점

처음에는 `App.jsx` 안에 화면을 모두 작성할 때는 기존 HTML과 비슷하게
느껴졌지만, 컴포넌트를 나누고 Props와 이벤트를 연결하면서 부모와 자식
사이에서 데이터가 어떻게 전달되는지 구분하는 부분이 헷갈렸다. 직접
컴포넌트를 분리하고 같은 컴포넌트에 다른 데이터를 전달해 보면서
React에서 컴포넌트를 재사용하는 방식이 조금 더 이해되었다.
