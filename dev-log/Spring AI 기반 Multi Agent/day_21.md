...
border-radius 이용하여 사진을 원으로 편집 할 수 있고 border를 이용해서 테두리 두께와 색, 종류 등을 지정 할 수 있다.

또한 box-shadow 이용해서 값을 가로 세로 흐림정도 번짐정도 그림자색상 등을 차례대로 값을 입력하면 정확한 결과값을 출력할 수 있다.

여기서 헷갈련던게 #container로 이미 중앙으로 정렬되어 있어서 추가적으로 따로 정의하고 조건을 입력해야 한다는 점을 모르고 시행착오를 겪었지만 오류를 해결했다.
