...
Spring Boot에서 Profile을 이용해 개발 환경과 테스트 환경을 분리하는
방법을 배웠다. 회원, 게시글, 권한 Entity의 연관관계를 설정하고
Repository, Service, Controller를 나누어 회원과 게시글의 조회, 등록,
수정, 삭제 기능을 구현하였다.

Entity를 화면에서 직접 사용하는 대신 DTO와 Form을 따로 만들어 데이터를
전달하고 입력받는 방법을 실습하였다. 또한 Validation과 `BindingResult`를
이용해 입력값을 검증하고, 필요한 경우 `rejectValue()`로 직접 오류를
추가하는 방법도 확인하였다.

Spring Security에서는 `UserDetails`를 이용해 데이터베이스의 회원 정보를
로그인 정보로 사용하고, `@AuthenticationPrincipal`을 이용해 현재
로그인한 회원의 정보를 가져오는 방법을 배웠다. BCrypt를 이용한 비밀번호
암호화와 기존 비밀번호 확인, 비밀번호 변경 과정도 함께 실습하였다.

## 느낀점

이번 실습은 회원가입, 로그인 사용자 정보, 회원 수정과 삭제, 비밀번호
변경, 게시글 CRUD까지 여러 기능이 한꺼번에 연결되어 있어서 이전보다
코드의 흐름을 따라가는 것이 어려웠다.

특히 Entity, DTO, Form이 각각 따로 있고 Controller와 Service까지
나누어져 있어서 처음에는 어떤 클래스에서 어떤 데이터를 처리하는지
헷갈렸다. 하지만 회원가입부터 게시글 작성까지 하나씩 흐름을 따라가
보면서 각 클래스가 맡는 역할을 조금씩 구분할 수 있었고, 로그인한 회원
정보가 실제 게시글 작성자와 연결되는 과정도 확인할 수 있었다.
